...
{{< x id=7269997868 user=secretGeek >}}
...
{{% notice info %}}
In this case, the example I had in mind was two algorithms for detecting whether shapes collide. One checks every possible combination of inputs while the second might use a [Quad Tree][quad-tree] or [Binary Search Partitioning][bsp] to only check items that are close to each other.
...
For example, say you're explaining an awesome new feature which will find the shortest path from A to B by generating a [navmesh][navmesh] and applying the [A* pathfinding algorithm][a-star].
...
Our company works in the CNC industry and for things that have some connection to the real world (e.g. say you're trying to implement [cut width compensation][kerf]) engineers are really good at analysing problems based in physics or geometry.
...
> Pretend there's a customer named Charlie who wants to buy a dozen fidget spinners. He goes to www.example.com, adds the fidget spinners to his cart then hits "checkout" (most people have used online shopping before so you can gloss over *how* a cart and checkout work).
>
...
This all sounds rather comical and the example is more than a bit contrived, but I can guarantee it's going to be more approachable than drawing up a big network diagram and using opaque words like "web server", "Event-Driven Architecture", or "Apache Kafka" (see the section on [techno-babble](#techno-babble)).
...
On one hand, if someone asks what you've been up to over the last week you could tell them the truth and say *"I added caching to `lookup_something()` to speed up common lookups and switched the algorithm in `detect_collisions()` from `O(n^2)` to `O(n log n)`"* but there's a good chance that'll mean absolutely nothing to them (see [techno-babble](#techno-babble)).

You could also say *"miscellaneous performance tweaks"*, but that sounds like a [cop out][co] and your manager will start questioning why they bother employing you.
...
{{% notice note %}}
If you're familiar with [Amdahl's Law][amdahl], improving the performance of something that isn't a bottleneck is kinda like throwing more cores at a problem that is inherently serial.
