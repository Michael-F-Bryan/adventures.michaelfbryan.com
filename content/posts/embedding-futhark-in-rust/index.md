...
It's not uncommon for a crate to need to build third-party non-Rust code or
ask `rustc` to link with native libraries. `cargo` lets you accomplish this via
a ["build script"][build-script], a small Rust program which is compiled and
executed *before* your main crate is compiled.

The ecosystem has already developed a variety of tools designed to make writing
build scripts easier. In particular, we'll be leveraging:

- [`cc`][cc] - for invoking the C compiler
- [`pkg-config`][pkg-config] - for querying `pkg-config` about our native
  dependencies
- [`bindgen`][bindgen] - to generate `unsafe` Rust declarations for our Futhark
  code

This will give us the tools we need to develop `futhark-build`, a build
script helper for compiling Futhark code and linking it to your Rust code.

This tool will

1. Compile our Futhark code to C with a user-specified code generator
2. Compile this C code to a library and emit the appropriate `-l` and `-L` flags
   so `rustc` can link to it
3. Use `bindgen` to generate a bunch of `unsafe` Rust bindings to the functions
   inside `mandelbrot.h`

{{% notice tip %}}
An important thing to keep in mind is that building and linking with native
libraries can vary a lot depending on the OS, or even just the way a user has
set up their system.

That means it's important to expose escape hatches that callers can use to
override parts of the build process!
{{% /notice %}}

I think the easiest way to create this sort of tool is by writing out our ideal
`build.rs` and working backwards.
...
The core of `futhark-build` is actually pretty simple. We just create a
builder type (`Build`), let the user set a couple options and configure how
particular steps are executed, then call the `generate()` method to compile
everything and place generated artefacts in a directory specified by `cargo`.

For now, the public API is incredibly simple.

```rust
// futhark-rs/futhark-build/src/lib.rs
...
impl Build {
    ...
    /// Compile the Futhark code and place all artefacts in `out_dir`.
...
Sometimes you'll need finer-grained control over the build process (e.g.
because you want to link dynamically instead of statically) or how bindings are
generated.

The `cc` and `bindgen` crates also use the builder pattern so I'm letting the
user pass in functions for manipulating the builders before they get finalised.
...
The `GeneratorError` is just a union of all the possible errors.
...
{{% notice tip %}}
We've annotated `GeneratorError` with the `#[non_exhaustive]` attribute to give
us space for future development.

The `GeneratorError::Other` variant can be used by the `configure_xxx()`
callbacks to notify the caller of an arbitrary error. Possible reasons could
be that you are using a different mechanism for finding dependencies (e.g.
`vcpkg` instead of `pkg-config` because you are on Windows) and that
mechanism returns an error.
{{% /notice %}}

Now we have a better understanding of the public API let's dive into the real
meat and potatoes of this crate, the `generate()` method.
...
You can see we've created methods for the each of the three steps from
earlier. The `name()` method inspects the filename passed into the
constructor and tries to extract the file stem part (e.g. `mandelbrot` in
`/path/to/mandelbrot.fut`). This lets all our generated artefacts use a
consistent naming scheme.

The `compile_futhark()` method constructs a call like
`futhark cuda --library mandelbrot.fut`, just like we did on the command line.
The only difference is we need to `match` on the `backend` to figure out which
code generator we need to invoke.
...
Next we need to compile the generated C code. This step is actually really
easy because we're offloading all the work to the `cc` crate.

All we need to do is create a `cc::Build` type and tell it what we'd like to
compile. This is also where that `configure_cc` hook gets used.
...
Generating bindings is made a bit more awkward because each builder method
will move the builder instead of mutating it (i.e. they take `self` instead
of `&mut self`). That can often make the API a bit clunkier than it needs to
be because you need to either put all the methods in one long method chain or
need to store it in a variable and continually update that variable with the
new value.

This gets particularly annoying when you have conditional logic where builder
methods are only used in certain code paths (e.g. a `match`).

I'm also not a fan of how you use a Rust string (`&str`) to specify the file
we are generating bindings. We are specifying a file on disk so you would
normally accept an `&OsStr` (or `&Path`, which is a newtype wrapper around
`&OsStr`) to specify any possible filename, not just those with a UTF-8
encoding.
...
We provide default values for `configure_cc` and `configure_bindgen` that just
include the relevant native libraries for the current backend, deferring to
the `pkg-config` crate to find these dependencies.
...
You should be able to figure out how `include_native_library()` and
`link_to_native_library()` work by looking at the docs for `pkg-config`, `cc`,
and `bindgen` so I won't bother copying them here.

Now we've seen how `futhark-build` works we can jump back to the `mandelbrot`
crate and see what it generated.
...
$ cargo build
   ...
   Compiling futhark-build v0.1.0 (https://github.com/Michael-F-Bryan/futhark-rs#aa3ac7b5)
   Compiling mandelbrot v0.1.0 (/home/michael/Documents/mandelbrot)
    Finished dev [unoptimized + debuginfo] target(s) in 25.26s

$ tree target/debug/build/mandelbrot-0ca98d725cb83fa9/
target/debug/build/mandelbrot-0ca98d725cb83fa9/
├── invoked.timestamp
├── out
│   ├── libmandelbrot.a
│   ├── mandelbrot.c
...
You can see the same `libmandelbrot.a`, `mandelbrot.c`, and `mandelbrot.h` were
generated as when we compiled manually.

The `mandelbrot.rs` file was generated by `bindgen` and contains several
thousand lines of Rust code.

```rust
// mandelbrot/target/debug/build/mandelbrot-0ca98d725cb83fa9/out/mandelbrot.rs
...
...
extern "C" {
    pub fn futhark_entry_rect_new(
        ctx: *mut futhark_context,
        out0: *mut *mut futhark_opaque_rect,
        in0: f64,
        in1: f64,
        in2: f64,
        in3: f64,
...
extern "C" {
    pub fn futhark_context_get_error(
        ctx: *mut futhark_context,
    ) -> *mut ::std::os::raw::c_char;
}
```

Now we just need to include the bindings in our actual `mandelbrot` crate.
...
{{% notice warning %}}
TODO: Write about the creation of `futhark-build` and our "ideal" build story.
{{% /notice %}}

## Exposing Futhark Code with Safe Rust

{{% notice warning %}}
TODO: Write about the `futhark` crate and how you can use it to write safe
interfaces.
{{% /notice %}}

## Using Futhark from Rust

{{% notice warning %}}
TODO: Obligatory demo + images.
{{% /notice %}}
...
[pkg-config]: https://lib.rs/pkg-config
